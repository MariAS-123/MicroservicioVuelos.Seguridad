...
public class ExceptionHandlingMiddleware
{
    ...
    public async Task InvokeAsync(HttpContext context)
    {
        ...
        catch (ValidationException ex)
        {
            ...
            await WriteErrorResponseAsync(context, StatusCodes.Status400BadRequest,
                ApiErrorResponse.Create(ex.Message, ex.Errors, context.TraceIdentifier));
        }
        catch (NotFoundException ex)
        {
            ...
            await WriteErrorResponseAsync(context, StatusCodes.Status404NotFound,
                ApiErrorResponse.Create(ex.Message, null, context.TraceIdentifier));
        }
        ...
        {
            _logger.LogWarning(ex, "Error de persistencia. TraceId: {TraceId}", context.TraceIdentifier);
            await WriteErrorResponseAsync(context, StatusCodes.Status409Conflict,
                ApiErrorResponse.Create(
                    "La operación no pudo completarse por una restricción de datos.",
                    null, context.TraceIdentifier));
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error interno. TraceId: {TraceId}", context.TraceIdentifier);
    ...
    private static async Task WriteErrorResponseAsync(
        HttpContext context, int statusCode, ApiErrorResponse response)
    {
